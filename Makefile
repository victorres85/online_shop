...
publish: ## Publish a build to the configured repo
	$(PIP) publish $(PIP_PUBLISH_OPTIONS_SET_BY_CI_ENV)

.PHONY: deps-py-update
deps-py-update: pyproject.toml ## Update PIP deps, e.g. after adding a new one manually
	$(PIP) update
